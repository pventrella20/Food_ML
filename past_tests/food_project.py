...
def kFold(X, y, model):
    ...
    ideal, _, _ = evaluate_model(X=X, y=y, cv=LeaveOneOut(), model=model)
    ...
    for k in folds:
        ...
        # evaluate k value
        k_mean, k_min, k_max = evaluate_model(X, y, cv, model)
        # report performance
    ...
    if model_type == 'regressor':
        for i in range(1, 101):
            ...
            for fold in folds:
                ...
                knn_2.fit(X_train_scaled, fold[2])
                predicted = knn_2.predict(X_test_scaled)
                y_pred = score_converter(predicted)
                y_test_2 = score_converter(fold[3])
                acc = accuracy_score(y_test_2, y_pred)
                scores.append(acc)
            results.append(mean(scores))
            print('Accuracy: %.3f (%.3f), nn = %d' % (mean(scores), std(scores), i))
    print('best NN number is {} ({:.2f}%)'.format(results.index(max(results))+1, max(results)))
...
def knnRegressor(df):
    ...
    # cross-validation
    kFold_cross_validation(X_food, y_food, 'regressor', 26)
    X_train, X_test, y_train, y_test = train_test_split(X_food, y_food, test_size=0.1, random_state=200)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)  # features normalization (train set)
    X_test_scaled = scaler.transform(X_test)  # features normalization (test set)
    knn = KNeighborsRegressor(n_neighbors=1)  # creo un knnRegressor
    knn.fit(X_train_scaled, y_train)  # fit del knn
